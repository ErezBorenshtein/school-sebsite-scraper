...
import pytz
from keepalive import kee_alive
  ...
  with open(csv_file, 'r') as f:
    ...
    for row in reader:
      if int(row[0]) == hour:
        ...
        used_rooms = set(
            row[day_index].split(',')) if row[day_index] else set()
        break
    else:
...
def get_day():
  ...
  day_of_week_int = current_date_israel.weekday(
  ) + 1  #! I dont know why the +1 is needed
  ...
  try:
    for hour, (start_time, end_time) in school_schedule.items():
      if start_time <= current_time <= end_time:
        ...
        difference = abs((start_time.hour * 60 + start_time.minute) -
...
async def on_message(message):
  ...
  if message.content.startswith('!empty classes'):
    ...
    if len(new_message) > 2 and new_message[2].lower(
    ) != "today":  #!empty classes [Day] [hour]
      classes = get_empty_rooms("schedule.csv", "class_numbers.json",
                                new_message[2], new_message[3])

      embed = discord.Embed(
          title=
          f"The empty rooms in {new_message[2]} at hour {new_message[3]} are:",
      ...
      #await message.channel.send(f"Empty classes: {', '.join(classes)}")
    else:
      if len(new_message) == 4:  #!empty classes today [hour]
        classes = get_empty_rooms("schedule.csv", "class_numbers.json",
                                  get_day(), new_message[3])
        embed = discord.Embed(
            title=
            f"The empty rooms today({get_day()}) at hour {new_message[3]} are:",
            description=', '.join(classes),
            color=discord.Color.blue())
        ...
        #await message.channel.send(
        #    f"Empty classes Today({get_day()}) at hour {new_message[3]}: {', '.join(classes)}"
        #)
      else:
        closest_hour = get_hour()
        if closest_hour is not None and get_day() != "Saturday":
          classes = get_empty_rooms("schedule.csv", "class_numbers.json",
                                    get_day(), closest_hour)
          ...
          await channel.send(embed=embed)
          #await message.channel.send(
          #    f"Empty classes Today({get_day()}) at hour{closest_hour}: {', '.join(classes)}"
          #)
        else:
          await message.channel.send(
              "Today is Saturday or school has finished today")
...
"""embed = discord.Embed(
      title="Bot is Ready!",
      description="I'm ready to rock and roll!",
      color=discord.Color.green()  # You can set the color of the embed here
  )

  # Set any additional fields if needed
  embed.add_field(name="Server Count",
                  value=f"I am in {len(client.guilds)} servers.")

  # Sending the embedded message to a specific channel
  channel_id = 1229734808507256835  # Replace YOUR_CHANNEL_ID with the actual channel ID
  channel = client.get_channel(channel_id)
  await channel.send(embed=embed) """
